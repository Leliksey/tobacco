    ...
    foreach ( $varia as $item ) {
        ...
        if ( $product->is_type( 'variable' ) ) {
            ...
            while ( $quantity > 0 ) {
                ...
                if ( $volume_item > 0 ) {
                    if ( $quantity == 1 ) break; // цикл не повинен повторюватись зайвий раз
                    $d_vol_all += $volume_item;
                } else {
                    $weighte = 'Об\'єм вказаний не для всіх товарів в кошику. Радимо уточнити цифри.';
                }                
                $quantity--;
            }
        }
...
if ( ( sizeof( $dimentions ) > 1 ) ) {
    ...
    if ( isset( $variations ) ) {
        if ( is_numeric( $variations[0]['dimensions']['length'] ) && 
             is_numeric( $variations[0]['dimensions']['width'] ) &&  
             is_numeric($variations[0]['dimensions']['height'])) {
                $alternate_vol = $variations[0]['dimensions']['length'] * 
                                 $variations[0]['dimensions']['width'] * 
                                 $variations[0]['dimensions']['height'];
            $volumemessage = '';
        }
    } else {
        $alternate_vol = $d_vol_all;
                ...
                <tbody>
                    ...
                    if (isset($billing_address[1])) {
                      ...
                      if ( $orderx->get_meta_data() ) {
                      	...
                      	$recipient_address_ref = $order_meta_data[1]->get_data()['value'];
                      }
