 ...
 class MNP_Plugin_Callbacks
 {
     ...
     public function morkvanpSelectRegion()
     {
        ...
        if (get_option('woocommerce_nova_poshta_shipping_method_area_name')) {
         ...
         echo '<p>Підказка: введіть перші 2-3 літери і дочекайтеся підвантаження даних з бази</p>';
     ...
     public function morkvanpWarehouseAddress()
     {
         ...
         echo '<p style="margin-left: 30px;">Підказка: введіть перші 2-3 літери і дочекайтеся підвантаження даних з бази</p>';
     ...
     public function morkvanpWarehouseAddress2()
     {
         ...
         echo '<input type=radio name=woocommerce_nova_poshta_sender_address_type value=1 '.$address_type.'><p class=afterradio>Якщо обрати адресу, поле для відділення не братиметься до уваги. <strong>Після створення накладної до кінця робочого дня вам зателефонують кур\'єри нової пошти та уточнюватимуть деталі щодо забору відправлень.</strong><br>У випадаючому списку будуть адреси, з яких Нова Пошта приймає, та адреси на які доставляє відправлення. </p>
    ...
    <input type="text"  name="woocommerce_nova_poshta_sender_flat" value="'.$sender_flat.'">
     ...
     public function morkvanpload()
     {
         ...
         $current = 1;
         $echo = false;
         echo '<input type="checkbox" class="regular-text" name="invoice_load" value="1" ' . checked($checked, $current, $echo) . ' />
      <p>Для прискорення сайту можна не вантажити зайві файли на всіх сторінках сайту. Проте позначивши цей пункт, якщо у вас one-step checkout, кастомна сторінка чекауту, або оформлення замовлення у спливаючому вікні доступне на всіх сторінках сайту, скрипти і стилі доведеться підключати окремо</p>';
     }
     ...
     public function morkvanpInvoiceupdatebases()
     {
         ...
         $current = 1;
         $echo = false;
         echo '<input '. $activate .' type="checkbox" class="regular-text" name="update_bases" value="1" ' . checked($checked, $current, $echo) . ' /><p>Якщо обрати цей пункт, оновлення баз відбуватиметься автоматично кожні 7 днів. </p>';
     }
     ...
     public function morkvanpcalc()
     {
         ...
         $current = 1;
         $echo = false;
         echo '<input '. $activate .' type="checkbox" class="regular-text" name="show_calc" value="1" ' . checked($checked, $current, $echo) . ' /> Вартість доставки буде показана біля способу доставки<br><small style="margin-left: 24px;"> Примітка. Сума доставки не включається у замовлення за замовчуванням.</small></p>';
     }
     ...
     public function morkvanpcalcplus()
     {
         ...
         $current = 1;
         $echo = false;
         echo '<input '. $activate .' type="checkbox" class="regular-text" name="plus_calc" value="1" ' . checked($checked, $current, $echo) . ' /> Вартість доставки буде додана до загальної суми замовлення<br><small style="margin-left: 24px;">Примітка. Можуть виникнути колізії з наложеним платежем.</small></p>';
     }
